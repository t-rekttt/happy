...
# No server build: the standalone runtime runs sources directly via tsx (see
# CMD), so the tsc/bundle step is unnecessary — and it would fail here because
# the server's specs import happy-app, which isn't in this build context.
